...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot showing both the PageFly-set preview image and what actually appears on share (paste link or attach the file directly in chat)",
  publish_status:
...
interface ImagePreviewNoteFields {
...
function formatImagePreviewNoteContent(
  fields: ImagePreviewNoteFields,
  ticketUrl: string
): string {
...
async function escalateImagePreviewIssueHandler(
  input: EscalateImagePreviewInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateImagePreviewOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatImagePreviewNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_image_preview_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_image_preview_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateImagePreviewIssueHandler, formatImagePreviewNoteContent };
