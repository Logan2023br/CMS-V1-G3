...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the broken background on mobile (paste link or attach the file directly in chat)",
  publish_status:
...
interface BackgroundMobileNoteFields {
...
function formatBackgroundMobileNoteContent(
  fields: BackgroundMobileNoteFields,
  ticketUrl: string
): string {
...
async function escalateBackgroundMobileIssueHandler(
  input: EscalateBackgroundMobileInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateBackgroundMobileOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateBackgroundMobileOutput;
  }
  ...
  if (missing.length > 0) {
    ...
    return {
      ...
      note_posted: false,
      note_post_error:
        "Not ready for escalation — Hugo MUST collect the real editor link, screenshot evidence (URL or attached file), and the publish_status answer. Do NOT fabricate URLs or status values.",
    };
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatBackgroundMobileNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_background_mobile_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_background_mobile_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateBackgroundMobileIssueHandler, formatBackgroundMobileNoteContent };
