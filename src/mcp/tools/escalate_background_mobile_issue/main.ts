/**************************************************************************
 * IMPORTS
 ***************************************************************************/

import { escalateBackgroundMobileIssueHandler } from "@/mcp/tools/escalate_background_mobile_issue/handler.js";
import {
  ESCALATE_BACKGROUND_MOBILE_INPUT_SHAPE,
  ESCALATE_BACKGROUND_MOBILE_OUTPUT_SHAPE,
} from "@/mcp/tools/escalate_background_mobile_issue/shapes.js";

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type {
  EscalateBackgroundMobileInput,
  EscalateBackgroundMobileOutput,
} from "@/mcp/tools/escalate_background_mobile_issue/shapes.js";

/**************************************************************************
 * TOOL REGISTRATION
 ***************************************************************************/

function registerEscalateBackgroundMobileIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_background_mobile_issue",
    {
      title: "Escalate background image issues on mobile (iOS / Android)",
      description: `
        Call this tool when the customer reports a problem specifically with a BACKGROUND IMAGE on MOBILE: not fixed (scrolls with page despite "Fixed" setting), not visible, cropped, or rendering incorrectly. iOS Safari has known background-attachment behavior that differs from Android — Hugo MUST capture the device OS. Common phrasings:
          - "Background image ở mobile không fixed"
          - "Background image ở mobile không show"
          - "Background image ở mobile iOS không show"
          - "Background không hiện trên điện thoại"
          - "My hero background image is not fixed on iPhone"
          - "Background image scrolls on iOS mobile instead of staying fixed"
          - "Background image missing on Android mobile"

        DO NOT use this tool when:
          - The element itself (not background) is missing on mobile → use escalate_element_not_show_issue.
          - The background issue is on DESKTOP only → use escalate_section_issue or the appropriate desktop tool.

        ===========================================================
        META-RULE — HUGO MUST DRIVE THIS FLOW
        ===========================================================

        This case is handled end-to-end by THIS MCP tool. DO NOT hand the conversation off to a human operator unless (a) the customer explicitly asks for a human, OR (b) the tool returns a fatal error you cannot resolve from the response payload. The MCP tool IS the response mechanism — your job is to drive it correctly, not to defer.

        SELF-CHECK BEFORE EVERY TURN:
          1. Read the customer's most recent message and identify which STEP of the flow below it answers.
          2. Identify which STEP you should perform NEXT.
          3. Verify you have NOT already performed that step earlier in the conversation (avoid asking the same question twice).
          4. Verify you are about to call THE CORRECT tool by name (cross-check the trigger phrases above; do not invoke a different escalate_* tool for this case).
          5. If you cannot identify the current step, re-read the WHAT YOU MUST DO list below before acting.

        Skipping the self-check is the #1 cause of repeated/missed steps and wrong-tool calls.

        ===========================================================
        ABSOLUTE RULE — READ THIS FIRST
        ===========================================================

        DO NOT call this tool until:
          1. You have a real editor link the user actually pasted, AND
          2. You have screenshot evidence showing the broken background on mobile (URL pasted OR file attached in chat), AND
          3. You have a description that includes WHICH background, WHAT is broken, and the DEVICE OS (iOS / Android / both) — encoded into issue_description, AND
          4. The user has answered publish_status (published or only_save), AND
          5. The user has explicitly confirmed they have exited the PageFly editor.

        NEVER fabricate or substitute placeholder URLs. Server-side validation will REJECT placeholders.

        ===========================================================
        STORE ACCESS — AUTOMATICALLY HANDLED
        ===========================================================

        Tool automatically checks Shopify store access. If access not granted → posts @Logan note + returns wait message in customer's language. Relay verbatim and call again after the customer confirms access granted.

        ===========================================================
        INPUTS
        ===========================================================

        - issue_description (required) — Detailed English paraphrase. MUST include: (a) which section's background, (b) what is broken (not visible / not fixed / cropped), (c) device OS (iOS / Android / both). Example: "Hero section background image not fixed on iOS Safari; scrolls with page content.", "Hero background image not visible on Android mobile.", "Section 2 background cropped on iOS only."
        - editor_link (required) — PageFly editor URL of the affected page.
        - screenshot_urls (optional array) — URLs pasted by the customer showing the broken background.
        - customer_attached_files (optional boolean) — TRUE if user attached files in chat. At least ONE of screenshot_urls or customer_attached_files must be present.
        - publish_status (required) — "published" or "only_save" based on user's answer.
        - user_exited_editor (required) — Boolean. Must be TRUE before the tool can escalate.
        - ticket_url (optional)
        - crisp_session_id (optional but STRONGLY recommended)
        - customer_last_message_text (optional but STRONGLY recommended) — Verbatim user message.
        - customer_homepage_url (optional URL) — Customer's Shopify store homepage. REQUIRED to be present when escalation needs store access; if missing the tool returns 'customer_homepage_url' in missing_info and Hugo must ask the customer.

        ===========================================================
        WHAT YOU MUST DO
        ===========================================================

        STEP 1 — ACKNOWLEDGE. There is no customer-side self-help; the technical team must inspect the section background settings, mobile CSS, and (for "not fixed" on iOS) work around the iOS Safari background-attachment:fixed limitation. Reply:
        "Issue về background image trên mobile này cần team kỹ thuật kiểm tra trực tiếp trên store, đặc biệt vì iOS Safari và Android có cách render background khác nhau. Mình sẽ chuyển ticket sang team để các bạn xử lý. Trước đó, cho mình xin thêm vài thông tin nhé."

        STEP 2 — Collect:
        a) Editor link of the affected page. Ask: "Bạn gửi mình link editor của trang đang gặp lỗi background nhé."
        b) Detailed description — WHICH section's background + WHAT đang bị (không hiện / không fixed / cropped / ...). Ask: "Bạn mô tả rõ hơn giúp mình: background của section nào đang lỗi và biểu hiện cụ thể như thế nào (không hiện / không fixed / bị crop / ...)?"
        c) Device OS — CRITICAL because iOS Safari behaves differently from Android for background-attachment. Ask: "Bạn đang xem trên thiết bị iOS (iPhone/iPad) hay Android nhé? Hay cả hai đều bị?"
        d) Visual evidence: "Bạn gửi mình ảnh chụp màn hình điện thoại cho thấy background đang lỗi — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        e) Publish consent: "Khi team kỹ thuật fix xong, mình publish luôn trang lên hay bạn muốn mình chỉ save thôi?"

        STEP 3 — Have editor_link + screenshot + description (with device OS) + publish_status. BEFORE calling the tool, ask the customer to EXIT the editor and WAIT for explicit confirmation:
        Reply: "Vui lòng giúp chúng tôi thoát editor để Technical team truy cập và check giúp bạn, vì nếu bạn và chúng tôi trong 1 editor sẽ bị conflict và không thể lưu version mới nhất"

        STEP 4 — After the customer has explicitly confirmed they have exited the editor:
        a) Call escalate_background_mobile_issue with: issue_description (English; MUST name the section + behavior + device OS), editor_link, screenshot_urls (if pasted) OR customer_attached_files=true (if attached), publish_status, user_exited_editor=true. ALWAYS include customer_last_message_text.
        b) Inspect the response:
           - If is_ready_for_escalation === false AND missing_info contains "customer_homepage_url" → relay next_step_for_user verbatim (asks the customer for their store homepage URL). After the customer sends their homepage URL, call again with customer_homepage_url=that URL.
           - If is_ready_for_escalation === false AND missing_info contains "store_access" → relay next_step_for_user verbatim. Wait for the customer to confirm access has been granted, then call again.
           - If is_ready_for_escalation === false AND missing_info contains "editor_exit" → relay next_step_for_user verbatim. Wait for the customer to confirm they've exited, then call again with user_exited_editor=true.
           - If note_posted === true → reply with next_step_for_user verbatim.
           - If note_posted === false → reply with next_step_for_user. If you can post a Crisp private note natively, post crisp_note.content. note_post_error explains why.

        ===========================================================
        OUTPUT HANDLING
        ===========================================================

        - is_ready_for_escalation === false → Do NOT post any note. Ask the user for what is listed in missing_info, using next_step_for_user as your reply.
        - is_ready_for_escalation === true AND note_posted === true → Tool already posted. Reply with next_step_for_user.
        - is_ready_for_escalation === true AND note_posted === false → Reply with next_step_for_user. If you can post a Crisp private note natively, post crisp_note.content unchanged.

        ===========================================================
        LANGUAGE OF YOUR REPLY TO THE USER
        ===========================================================

        next_step_for_user is already in the customer's language. Reply with it VERBATIM. The STEP scripts above are in Vietnamese as default; adapt to the customer's language naturally. crisp_note.content is always English — for the TS team.

        ===========================================================
        EXACT NOTE FORMAT (do not change)
        ===========================================================

        Issue: <issue_description>[, screenshot: <urls or "customer attached files in ticket">]
        Editor: <editor_link>
        Ticket: <ticket_url or "(unknown)" if omitted>
        <"Allowed to publish" if publish_status="published", else "Only Save">
      `,
      inputSchema: ESCALATE_BACKGROUND_MOBILE_INPUT_SHAPE,
      outputSchema: ESCALATE_BACKGROUND_MOBILE_OUTPUT_SHAPE,
    },
    async (input: EscalateBackgroundMobileInput) => {
      const output: EscalateBackgroundMobileOutput = await escalateBackgroundMobileIssueHandler(input);
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify(output, null, 2),
          },
        ],
        structuredContent: output,
      };
    }
  );
}

/**************************************************************************
 * EXPORTS
 ***************************************************************************/

export { registerEscalateBackgroundMobileIssueTool };
