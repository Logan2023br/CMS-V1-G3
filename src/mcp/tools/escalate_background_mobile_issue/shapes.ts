...
const ESCALATE_BACKGROUND_MOBILE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST name (a) which background image element is affected (section/row name), (b) what is broken (not visible / not fixed / scrolling instead of fixed / cropped), (c) the customer's device OS (iOS / Android / both). Example: 'Hero section background image not fixed on iOS Safari; scrolls with page content.', 'Background image not visible on Android mobile.', 'Hero background image rendering cropped on iOS only.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the broken background on mobile. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true so TS can confirm the visual behavior."
    ),
...
type EscalateBackgroundMobileInput = z.infer<typeof ESCALATE_BACKGROUND_MOBILE_INPUT_SHAPE>;
...
const ESCALATE_BACKGROUND_MOBILE_OUTPUT_SHAPE = z.object({
...
type EscalateBackgroundMobileOutput = z.infer<typeof ESCALATE_BACKGROUND_MOBILE_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateBackgroundMobileInput,
  type EscalateBackgroundMobileOutput,
};
