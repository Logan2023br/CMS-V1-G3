...
function formatFormNoteContent(
...
async function escalateFormIssueHandler(
  input: EscalateFormInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateFormOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatFormNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_form_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_form_issue] match: posted=false error=${noteResult.error}`
    );
  }
