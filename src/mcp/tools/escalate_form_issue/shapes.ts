...
const ESCALATE_FORM_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's detailed English paraphrase. Identify WHAT is broken about the form and (if customer requested) WHAT they want changed. Examples: 'Form submit does not deliver data to TS team — submissions are blank.', 'Customer wants the post-submit confirmation text to appear above the form instead of below.', 'Form data is missing fields after submission.'"
    ),
  ...
  editor_link: z
    ...
    .url()
    .describe(
      "The PageFly editor URL of the page that contains the form. Take what the user pasted. No placeholders."
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the form / the broken behavior (screenshot of the form, missing-data sample, or recording of the submit). Omit if the user attached files directly in chat."
    ),
...
type EscalateFormInput = z.infer<typeof ESCALATE_FORM_INPUT_SHAPE>;
...
const ESCALATE_FORM_OUTPUT_SHAPE = z.object({
...
type EscalateFormOutput = z.infer<typeof ESCALATE_FORM_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateFormInput,
  type EscalateFormOutput,
};
