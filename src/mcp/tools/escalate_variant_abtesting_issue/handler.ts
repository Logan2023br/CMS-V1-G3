...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  editor_link: "the editor link for the affected A/B-testing page",
...
interface VariantAbTestingNoteFields {
...
function formatVariantAbTestingNoteContent(
  fields: VariantAbTestingNoteFields,
  ticketUrl: string
): string {
...
async function escalateVariantAbTestingIssueHandler(
  input: EscalateVariantAbTestingInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateVariantAbTestingOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateVariantAbTestingOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatVariantAbTestingNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_variant_abtesting_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_variant_abtesting_issue] match: posted=false error=${noteResult.error}`
    );
  }
