...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot showing the product in editor and the live page where it is missing (paste link or attach the file directly in chat)",
...
interface ProductShowNoteFields {
...
function formatProductShowNoteContent(
  fields: ProductShowNoteFields,
  ticketUrl: string
): string {
...
async function escalateProductShowIssueHandler(
  input: EscalateProductShowInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateProductShowOutput> {
  ...
  if (missing.length > 0) {
    ...
    return {
      ...
      note_posted: false,
      note_post_error:
        "Not ready for escalation — Hugo MUST collect a real editor link, screenshot evidence, AND explicit user consent to publish. Fix verification requires publish.",
    };
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatProductShowNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_product_show_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_product_show_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateProductShowIssueHandler, formatProductShowNoteContent };
