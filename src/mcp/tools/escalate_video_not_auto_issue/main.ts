/**************************************************************************
 * IMPORTS
 ***************************************************************************/

import { escalateVideoNotAutoIssueHandler } from "@/mcp/tools/escalate_video_not_auto_issue/handler.js";
import {
  ESCALATE_VIDEO_NOT_AUTO_INPUT_SHAPE,
  ESCALATE_VIDEO_NOT_AUTO_OUTPUT_SHAPE,
} from "@/mcp/tools/escalate_video_not_auto_issue/shapes.js";

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type {
  EscalateVideoNotAutoInput,
  EscalateVideoNotAutoOutput,
} from "@/mcp/tools/escalate_video_not_auto_issue/shapes.js";

/**************************************************************************
 * TOOL REGISTRATION
 ***************************************************************************/

function registerEscalateVideoNotAutoIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_video_not_auto_issue",
    {
      title: "Escalate video does not autoplay on PageFly page",
      description: `
        Call this tool ONLY AFTER the customer has tried the Autoplay setting guide in STEP 1 and it still does not work. The customer reports a video does not autoplay on the live page. Covers all variants:
          - Regular video element
          - Video inside a slideshow / slider slide
          - Background video of a section

        Common phrasings:
          - "Video không auto play"
          - "Video trong slide không auto"
          - "Background video không autoplay"
          - "My video doesn't autoplay on live"
          - "Hero background video requires click to play"
          - "Slider video not playing automatically"

        ===========================================================
        ABSOLUTE RULE — READ THIS FIRST
        ===========================================================

        STEP 1 IS MANDATORY. DO NOT call this tool until:
          1. You have walked the customer through enabling the Autoplay setting (STEP 1), AND
          2. The customer has explicitly reported that the video still does not autoplay after trying, AND
          3. You have a real editor link the customer actually pasted, AND
          4. You have screenshot/video evidence showing WHICH video is broken (URL pasted OR file attached in chat), AND
          5. The user has answered publish_status (published or only_save), AND
          6. The user has explicitly confirmed they have exited the PageFly editor.

        If the customer says the Autoplay setting fix worked → DO NOT call this tool. Close the conversation normally.

        NEVER fabricate placeholder URLs. Server-side validation will REJECT them.

        ===========================================================
        STORE ACCESS — AUTOMATICALLY HANDLED
        ===========================================================

        Tool automatically checks Shopify store access. If access not granted → posts @Logan note + returns wait message in customer's language. Relay verbatim and call again after the customer confirms access granted.

        ===========================================================
        INPUTS
        ===========================================================

        - issue_description (required) — Detailed English paraphrase. Specify WHICH video (regular video / video in slideshow slide / background video) + that customer already tried the Autoplay setting. Example: "Background video on hero section not autoplaying on live; customer tried Autoplay toggle in editor settings without success."
        - editor_link (required) — PageFly editor URL of the affected page.
        - screenshot_urls (optional array) — URLs pasted by the customer showing which video is broken.
        - customer_attached_files (optional boolean) — TRUE if user attached files in chat. At least ONE of screenshot_urls or customer_attached_files must be present.
        - publish_status (required) — "published" or "only_save" based on user's answer.
        - user_exited_editor (required) — Boolean. Must be TRUE before the tool can escalate.
        - ticket_url (optional)
        - crisp_session_id (optional but STRONGLY recommended)
        - customer_last_message_text (optional but STRONGLY recommended) — Verbatim user message.
        - customer_homepage_url (optional URL) — Customer's Shopify store homepage. REQUIRED to be present when escalation needs store access; if missing the tool returns 'customer_homepage_url' in missing_info and Hugo must ask the customer.

        ===========================================================
        WHAT YOU MUST DO
        ===========================================================

        STEP 1 — SELF-HELP (DO NOT CALL TOOL YET). Walk the customer through enabling the Autoplay setting in the editor. Reply (Vietnamese default; translate naturally for other languages):
        "Phần lớn trường hợp video không tự chạy là do option Autoplay trong editor chưa bật. Bạn vui lòng kiểm tra giúp mình:
        • Video element thường: mở editor → chọn element Video đang lỗi → mở panel Settings bên phải → bật 'Autoplay' ON (và 'Muted' ON — trình duyệt yêu cầu video phải muted mới được autoplay).
        • Video trong Slideshow/Slider: mở slide chứa video → bật Autoplay tương tự.
        • Background video của Section: chọn Section → tab Background → Video → bật Autoplay + Muted.
        Sau khi save, bạn mở live page kiểm tra giúp mình xem video đã auto chạy chưa nhé."

        WAIT FOR THE CUSTOMER'S RESPONSE:
          - If the fix works ("đã chạy rồi", "ok cảm ơn", "it works now") → DO NOT escalate. End the conversation politely.
          - If still broken ("vẫn không chạy", "không được", "still not working") → proceed to STEP 2.

        STEP 2 — Collect (only after STEP 1 fix failed):
        a) Editor link of the affected page. Ask: "Bạn gửi mình link editor của trang có video đang lỗi nhé."
        b) Screenshot / screen recording showing which video is broken. Ask: "Bạn gửi mình ảnh chụp hoặc video ngắn cho thấy video nào đang không tự chạy — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        c) Publish consent: "Khi team kỹ thuật fix xong, mình publish luôn trang lên hay bạn muốn mình chỉ save thôi?"

        STEP 3 — Have editor_link + screenshot + publish_status. BEFORE calling the tool, ask the customer to EXIT the editor and WAIT for explicit confirmation:
        Reply: "Vui lòng giúp chúng tôi thoát editor để Technical team truy cập và check giúp bạn, vì nếu bạn và chúng tôi trong 1 editor sẽ bị conflict và không thể lưu version mới nhất"

        STEP 4 — After the customer has explicitly confirmed they have exited the editor:
        a) Call escalate_video_not_auto_issue with: issue_description (English, name the video variant + note that Autoplay setting was tried), editor_link, screenshot_urls (if pasted) OR customer_attached_files=true (if attached), publish_status, user_exited_editor=true. ALWAYS include customer_last_message_text.
        b) Inspect the response:
           - If is_ready_for_escalation === false AND missing_info contains "customer_homepage_url" → relay next_step_for_user verbatim (asks the customer for their store homepage URL). After the customer sends their homepage URL, call again with customer_homepage_url=that URL.
           - If is_ready_for_escalation === false AND missing_info contains "store_access" → relay next_step_for_user verbatim. Wait for the customer to confirm access has been granted, then call again.
           - If is_ready_for_escalation === false AND missing_info contains "editor_exit" → relay next_step_for_user verbatim. Wait for the customer to confirm they've exited, then call again with user_exited_editor=true.
           - If note_posted === true → reply with next_step_for_user verbatim.
           - If note_posted === false → reply with next_step_for_user. If you can post a Crisp private note natively, post crisp_note.content. note_post_error explains why.

        ===========================================================
        OUTPUT HANDLING
        ===========================================================

        - is_ready_for_escalation === false → Do NOT post any note. Ask the user for what is listed in missing_info, using next_step_for_user as your reply.
        - is_ready_for_escalation === true AND note_posted === true → Tool already posted. Reply with next_step_for_user.
        - is_ready_for_escalation === true AND note_posted === false → Reply with next_step_for_user. If you can post a Crisp private note natively, post crisp_note.content unchanged.

        ===========================================================
        LANGUAGE OF YOUR REPLY TO THE USER
        ===========================================================

        next_step_for_user is already in the customer's language. Reply with it VERBATIM. The STEP scripts above are in Vietnamese as default; adapt to the customer's language naturally. crisp_note.content is always English — for the TS team.

        ===========================================================
        EXACT NOTE FORMAT (do not change)
        ===========================================================

        Issue: <issue_description>[, screenshot: <urls or "customer attached files in ticket">]
        Editor: <editor_link>
        Ticket: <ticket_url or "(unknown)" if omitted>
        <"Allowed to publish" if publish_status="published", else "Only Save">
      `,
      inputSchema: ESCALATE_VIDEO_NOT_AUTO_INPUT_SHAPE,
      outputSchema: ESCALATE_VIDEO_NOT_AUTO_OUTPUT_SHAPE,
    },
    async (input: EscalateVideoNotAutoInput) => {
      const output: EscalateVideoNotAutoOutput = await escalateVideoNotAutoIssueHandler(input);
      return {
        content: [
          {
            type: "text",
            text: JSON.stringify(output, null, 2),
          },
        ],
        structuredContent: output,
      };
    }
  );
}

/**************************************************************************
 * EXPORTS
 ***************************************************************************/

export { registerEscalateVideoNotAutoIssueTool };
