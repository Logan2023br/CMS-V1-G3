...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot or screen recording showing which video is not autoplaying (paste link or attach the file directly in chat)",
  publish_status:
...
interface VideoNotAutoNoteFields {
...
function formatVideoNotAutoNoteContent(
  fields: VideoNotAutoNoteFields,
  ticketUrl: string
): string {
...
async function escalateVideoNotAutoIssueHandler(
  input: EscalateVideoNotAutoInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateVideoNotAutoOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatVideoNotAutoNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_video_not_auto_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_video_not_auto_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateVideoNotAutoIssueHandler, formatVideoNotAutoNoteContent };
