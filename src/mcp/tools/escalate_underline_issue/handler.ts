...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the underlined text location (paste link or attach the file directly in chat)",
  publish_status:
...
function formatUnderlineNoteContent(
  fields: UnderlineNoteFields,
  ticketUrl: string
): string {
...
async function escalateUnderlineIssueHandler(
  input: EscalateUnderlineInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateUnderlineOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatUnderlineNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_underline_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_underline_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateUnderlineIssueHandler, formatUnderlineNoteContent };
