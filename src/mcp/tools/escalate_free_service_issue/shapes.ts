...
const ESCALATE_FREE_SERVICE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the request, ALWAYS IN ENGLISH. MUST include: (a) which feature the customer wants added (Marquee Text / Marquee Image / Show Variant Name / Hidden Variant Unavailable / Show Less-More Variant / Show Shopify SKU / Show Shopify Basic Price / Show Save-Price / Scroll-to-Top button / other), (b) the customer has accepted the free service offer, (c) where in the page they want it added, (d) any visual effect description. Example: 'Customer accepted free custom-code service; wants Marquee Text effect added under Hero section, scrolling right-to-left.', 'Free service accepted; add Show Save-Price element under the product price block.'"
    ),
  ...
  reference_urls: z
    ...
    .optional()
    .describe(
      "OPTIONAL — URLs the customer pasted as reference (effect demo link, mockup image, screen recording, etc.). Include if customer provides; not required."
    ),
  ...
  publish_status: z
    ...
    .describe(
      "Customer's explicit answer: 'published' = TS may publish the page after adding the feature, 'only_save' = TS must only save without publishing. Ask first if you have not."
    ),
...
type EscalateFreeServiceInput = z.infer<typeof ESCALATE_FREE_SERVICE_INPUT_SHAPE>;
...
const ESCALATE_FREE_SERVICE_OUTPUT_SHAPE = z.object({
  issue_summary: z.string(),

  is_ready_for_escalation: z
    .boolean()
    .describe(
      "True iff editor_link is provided AND publish_status is set AND store access is granted AND the customer has exited the editor. Reference media is OPTIONAL."
    ),
...
type EscalateFreeServiceOutput = z.infer<typeof ESCALATE_FREE_SERVICE_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateFreeServiceInput,
  type EscalateFreeServiceOutput,
};
