...
interface PublishLiquidErrorNoteFields {
...
function formatPublishLiquidErrorNoteContent(
  fields: PublishLiquidErrorNoteFields,
  ticketUrl: string
): string {
...
async function escalatePublishLiquidErrorIssueHandler(
  input: EscalatePublishLiquidErrorInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalatePublishLiquidErrorOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalatePublishLiquidErrorOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatPublishLiquidErrorNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_publish_liquid_error_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_publish_liquid_error_issue] match: posted=false error=${noteResult.error}`
    );
  }
