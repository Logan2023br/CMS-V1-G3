...
import { escalatePublishLiquidErrorIssueHandler } from "@/mcp/tools/escalate_publish_liquid_error_issue/handler.js";
import {
  ESCALATE_PUBLISH_LIQUID_ERROR_INPUT_SHAPE,
  ESCALATE_PUBLISH_LIQUID_ERROR_OUTPUT_SHAPE,
} from "@/mcp/tools/escalate_publish_liquid_error_issue/shapes.js";

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type {
  EscalatePublishLiquidErrorInput,
  EscalatePublishLiquidErrorOutput,
} from "@/mcp/tools/escalate_publish_liquid_error_issue/shapes.js";
...
function registerEscalatePublishLiquidErrorIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_publish_liquid_error_issue",
    {
      title: "Escalate 'liquid error on publish' (likely 256kb page-size limit)",
      description: `
        Call this tool when the user reports a liquid error when trying to publish a page or section. Common phrasings:
          - "Publish page nhưng nó hiện lỗi liquid error"
          - "Publish section bị báo liquid error"
          - "Page vượt quá 256kb và show lỗi liquid khi publish"
          - "Cannot publish — Shopify says liquid error"
          - Any publish-time liquid-error complaint (often the error image clearly shows the word "Liquid").
        ...
        DO NOT call this tool until:
          1. You have given the STEP 1 explanation (256kb page-size cause + suggestion to reduce content), AND
        ...
        - issue_description (required) — One-line English paraphrase. Mention the customer was informed about the 256kb cause + requested technical confirmation. Example: "Liquid error on publish; customer informed about 256kb page-size limit, requested technical confirmation."
        - editor_link (required) — PageFly editor URL the user pasted.
        - screenshot_urls (optional array) — URLs the user pasted showing the liquid error message.
        ...
        STEP 1 — INFORM. Tell the customer about the most common cause and a quick suggestion. After this, proceed to STEP 2 regardless of the customer's response — the team always confirms technically. Reply:

        "Lỗi này thường xuất hiện do page size của bạn vượt quá 256kb. Để fix, bạn có thể thử giảm content và publish lại. Tuy nhiên để đảm bảo chính xác 100%, mình sẽ chuyển ticket sang team kỹ thuật kiểm tra và xác nhận giúp bạn nhé."

        Then proceed to STEP 2.

        STEP 2 — Collect:
        a) Editor link of the affected page/section. Ask: "Bạn gửi mình link editor của trang đang publish bị lỗi nhé."
        b) Evidence (OPTIONAL but helpful): "Nếu được, bạn gửi mình ảnh chụp đoạn liquid error đang hiện ra khi publish — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        ...
        a) Call escalate_publish_liquid_error_issue with: issue_description (English, mention 256kb informed + tech confirmation requested), editor_link, user_consented_to_publish=true, user_exited_editor=true. If user pasted screenshot URLs include them in screenshot_urls. If user attached files in chat set customer_attached_files=true. ALWAYS include customer_last_message_text.
      ...
      `,
      inputSchema: ESCALATE_PUBLISH_LIQUID_ERROR_INPUT_SHAPE,
      outputSchema: ESCALATE_PUBLISH_LIQUID_ERROR_OUTPUT_SHAPE,
    },
    async (input: EscalatePublishLiquidErrorInput) => {
      const output: EscalatePublishLiquidErrorOutput = await escalatePublishLiquidErrorIssueHandler(input);
      return {
        content: [
          {
            type: "text",
...
export { registerEscalatePublishLiquidErrorIssueTool };
