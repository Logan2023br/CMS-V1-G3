...
const ESCALATE_PUBLISH_LIQUID_ERROR_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. Mention the customer was informed about the 256kb page-size cause + suggestion to reduce content. Example: 'Liquid error on publish — customer informed about 256kb page-size limit, requested technical confirmation.', 'Publish fails with liquid error; page size suspected over 256kb.'"
    ),
  ...
  editor_link: z
    ...
    .url()
    .describe(
      "The PageFly editor URL of the affected page/section. Take what the user pasted. No placeholders."
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the liquid error message. Omit if the user attached files directly in chat."
    ),
...
type EscalatePublishLiquidErrorInput = z.infer<typeof ESCALATE_PUBLISH_LIQUID_ERROR_INPUT_SHAPE>;
...
const ESCALATE_PUBLISH_LIQUID_ERROR_OUTPUT_SHAPE = z.object({
...
type EscalatePublishLiquidErrorOutput = z.infer<typeof ESCALATE_PUBLISH_LIQUID_ERROR_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalatePublishLiquidErrorInput,
  type EscalatePublishLiquidErrorOutput,
};
