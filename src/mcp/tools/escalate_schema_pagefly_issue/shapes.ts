...
const ESCALATE_SCHEMA_PAGEFLY_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's detailed English paraphrase. Identify ACTION (add / remove / modify schema) + SCHEMA CONTENT (paste verbatim if customer provided schema code to add) OR LOCATION (which schema to remove). Examples: 'Customer wants to add Product schema (JSON-LD code provided in conversation) to PageFly product page.', 'Customer wants to remove an existing FAQ schema from product page (screenshot attached showing the schema in source).'"
    ),
  ...
  editor_link: z
    ...
    .url()
    .describe(
      "The PageFly editor URL of the affected product page. Take what the user pasted. No placeholders."
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted (screenshot of the schema to remove, or visual proof of what they want changed). Omit if the user attached files directly in chat."
    ),
...
type EscalateSchemaPageflyInput = z.infer<typeof ESCALATE_SCHEMA_PAGEFLY_INPUT_SHAPE>;
...
const ESCALATE_SCHEMA_PAGEFLY_OUTPUT_SHAPE = z.object({
...
type EscalateSchemaPageflyOutput = z.infer<typeof ESCALATE_SCHEMA_PAGEFLY_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateSchemaPageflyInput,
  type EscalateSchemaPageflyOutput,
};
