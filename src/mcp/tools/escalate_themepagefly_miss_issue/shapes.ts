...
const ESCALATE_THEMEPAGEFLY_MISS_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. Example: 'Publish fails with error about missing theme.pagefly.liquid file.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the missing-theme.pagefly.liquid error (screenshot of the error message). Omit if the user attached files directly in chat."
    ),
...
type EscalateThemePageflyMissInput = z.infer<typeof ESCALATE_THEMEPAGEFLY_MISS_INPUT_SHAPE>;
...
const ESCALATE_THEMEPAGEFLY_MISS_OUTPUT_SHAPE = z.object({
...
type EscalateThemePageflyMissOutput = z.infer<typeof ESCALATE_THEMEPAGEFLY_MISS_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateThemePageflyMissInput,
  type EscalateThemePageflyMissOutput,
};
