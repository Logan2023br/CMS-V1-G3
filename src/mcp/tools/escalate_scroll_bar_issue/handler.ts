...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot showing the section/block with the visible scrollbar to hide (paste link or attach the file directly in chat)",
  publish_status:
...
function formatScrollBarNoteContent(
  fields: ScrollBarNoteFields,
  ticketUrl: string
): string {
...
async function escalateScrollBarIssueHandler(
  input: EscalateScrollBarInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateScrollBarOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatScrollBarNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_scroll_bar_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_scroll_bar_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateScrollBarIssueHandler, formatScrollBarNoteContent };
