  ...
  server.registerTool(
    "escalate_section_issue",
    {
      ...
      description: `
        ...
        STEP 3 — Have editor_link + user said YES to publish. Reference media optional.
        a) Call escalate_section_issue with: issue_description (English, mention Section vs Page), editor_link, user_consented_to_publish=true. If user pasted reference URLs include them in reference_urls. If user attached files directly in chat set customer_attached_files=true (and omit/empty reference_urls). If both, pass both. ALWAYS include customer_last_message_text.
