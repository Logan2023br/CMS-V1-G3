...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the text/element showing the wrong font (paste link or attach the file directly in chat)",
  publish_status:
...
function formatFontNoteContent(
  fields: FontNoteFields,
  ticketUrl: string
): string {
...
async function escalateFontIssueHandler(
  input: EscalateFontInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateFontOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatFontNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_font_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_font_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateFontIssueHandler, formatFontNoteContent };
