...
const ESCALATE_FONT_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST classify the symptom: (a) wrong font in editor, (b) wrong font on live, (c) font cannot be updated, (d) editor vs live mismatch. Name the affected text/element if known. Example: 'Heading font on live page does not match what is set in PageFly editor; editor shows correct font but live falls back to theme default.', 'Customer cannot apply selected font to body text; font picker save fails silently.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the wrong/desired font location (annotated if possible). Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true."
    ),
...
type EscalateFontInput = z.infer<typeof ESCALATE_FONT_INPUT_SHAPE>;
...
const ESCALATE_FONT_OUTPUT_SHAPE = z.object({
...
type EscalateFontOutput = z.infer<typeof ESCALATE_FONT_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateFontInput,
  type EscalateFontOutput,
};
