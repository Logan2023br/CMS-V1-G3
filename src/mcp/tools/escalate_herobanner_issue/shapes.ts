...
const ESCALATE_HEROBANNER_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST describe: (a) which banner / section is affected, (b) current sizing behavior (cropped / not full width / wrong height / not matching viewport), (c) what the customer wants (full screen / full viewport width / specific aspect ratio). Example: 'Hero banner background image not rendering at full viewport width on desktop; customer wants edge-to-edge full-screen.', 'Hero banner background image rendering smaller than intended size; customer wants full-screen viewport sizing.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the broken banner sizing. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true so TS can confirm the current vs desired sizing."
    ),
...
type EscalateHerobannerInput = z.infer<typeof ESCALATE_HEROBANNER_INPUT_SHAPE>;
...
const ESCALATE_HEROBANNER_OUTPUT_SHAPE = z.object({
...
type EscalateHerobannerOutput = z.infer<typeof ESCALATE_HEROBANNER_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateHerobannerInput,
  type EscalateHerobannerOutput,
};
