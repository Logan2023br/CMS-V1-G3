...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot showing the current banner sizing and the desired sizing (paste link or attach the file directly in chat)",
  publish_status:
...
interface HerobannerNoteFields {
...
function formatHerobannerNoteContent(
  fields: HerobannerNoteFields,
  ticketUrl: string
): string {
...
async function escalateHerobannerIssueHandler(
  input: EscalateHerobannerInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateHerobannerOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatHerobannerNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_herobanner_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_herobanner_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateHerobannerIssueHandler, formatHerobannerNoteContent };
