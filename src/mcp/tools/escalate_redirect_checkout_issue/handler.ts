...
interface RedirectCheckoutNoteFields {
...
function formatRedirectCheckoutNoteContent(
  fields: RedirectCheckoutNoteFields,
  ticketUrl: string
): string {
...
async function escalateRedirectCheckoutIssueHandler(
  input: EscalateRedirectCheckoutInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateRedirectCheckoutOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateRedirectCheckoutOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatRedirectCheckoutNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_redirect_checkout_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_redirect_checkout_issue] match: posted=false error=${noteResult.error}`
    );
  }
