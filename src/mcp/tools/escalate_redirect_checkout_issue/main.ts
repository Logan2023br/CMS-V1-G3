...
import { escalateRedirectCheckoutIssueHandler } from "@/mcp/tools/escalate_redirect_checkout_issue/handler.js";
import {
  ESCALATE_REDIRECT_CHECKOUT_INPUT_SHAPE,
  ESCALATE_REDIRECT_CHECKOUT_OUTPUT_SHAPE,
} from "@/mcp/tools/escalate_redirect_checkout_issue/shapes.js";

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type {
  EscalateRedirectCheckoutInput,
  EscalateRedirectCheckoutOutput,
} from "@/mcp/tools/escalate_redirect_checkout_issue/shapes.js";
...
function registerEscalateRedirectCheckoutIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_redirect_checkout_issue",
    {
      title: "Escalate ATC / Checkout button redirecting to home page",
      description: `
        Call this tool when the customer reports clicking an Add-to-Cart (ATC), Checkout, or Buy-now button on a PageFly page redirects them to the home page (or any wrong destination) instead of opening the cart / proceeding to checkout. Common phrasings:
          - "Click button ATC tự động redirect về home page"
          - "Click Checkout nhưng nó về trang chủ"
          - "Bấm Buy now không vào checkout, lại quay về home"
          - "ATC button redirects to homepage instead of opening cart"

        DO NOT use this tool when the cart drawer fails to open at all (use escalate_cart_drawer_issue) — this tool is specifically for the WRONG-REDIRECT behavior after click.
        ...
        DO NOT call this tool until:
          ...
          2. You have a detailed description of WHICH button is affected and the redirect behavior (this goes into issue_description), AND
        ...
        - issue_description (required) — Detailed English paraphrase. Identify WHICH button (ATC / Checkout / Buy now) + WHAT happens. Example: "ATC button on product page redirects to home page instead of opening cart."
        - editor_link (required) — PageFly editor URL the user pasted.
        - screenshot_urls (optional array) — URLs the user pasted showing the broken button (screenshot pointing at it, short recording of the redirect).
        ...
        STEP 1 — ACKNOWLEDGE. There is no customer-side self-help; the technical team must inspect the button's link / JS. Reply:
        "Mình sẽ chuyển ticket sang team kỹ thuật để các bạn kiểm tra button bị redirect sai cho bạn. Trước đó, cho mình xin thêm vài thông tin nhé."

        STEP 2 — Collect:
        a) Editor link of the affected page. Ask: "Bạn gửi mình link editor của trang đang gặp lỗi nhé."
        b) Detailed description. Ask: "Bạn cho mình biết button nào đang lỗi (ATC, Checkout, Buy now, ...) và khi click thì bị redirect đi đâu nhé."
        c) Visual evidence (OPTIONAL but helpful): "Nếu được, bạn gửi mình ảnh chụp hoặc video ngắn cho thấy button đang lỗi — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        ...
        a) Call escalate_redirect_checkout_issue with: issue_description (English, mention WHICH button + WHERE it redirects), editor_link, user_consented_to_publish=true, user_exited_editor=true. If user pasted screenshot URLs include them in screenshot_urls. If user attached files in chat set customer_attached_files=true. ALWAYS include customer_last_message_text.
      ...
      `,
      inputSchema: ESCALATE_REDIRECT_CHECKOUT_INPUT_SHAPE,
      outputSchema: ESCALATE_REDIRECT_CHECKOUT_OUTPUT_SHAPE,
    },
    async (input: EscalateRedirectCheckoutInput) => {
      const output: EscalateRedirectCheckoutOutput = await escalateRedirectCheckoutIssueHandler(input);
      return {
        content: [
          {
            type: "text",
...
export { registerEscalateRedirectCheckoutIssueTool };
