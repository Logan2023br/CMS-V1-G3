...
const ESCALATE_REDIRECT_CHECKOUT_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's detailed English paraphrase. Identify WHICH button is affected (Add-to-cart / Checkout / Buy now) and WHAT actually happens (e.g. redirects to home page instead of cart/checkout). Example: 'ATC button on product page redirects customer to home page instead of opening cart.', 'Checkout button does not go to Shopify checkout — redirects to home page.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing which button is broken (screenshot of the button or short recording of the redirect happening). Omit if the user attached files directly in chat."
    ),
...
type EscalateRedirectCheckoutInput = z.infer<typeof ESCALATE_REDIRECT_CHECKOUT_INPUT_SHAPE>;
...
const ESCALATE_REDIRECT_CHECKOUT_OUTPUT_SHAPE = z.object({
...
type EscalateRedirectCheckoutOutput = z.infer<typeof ESCALATE_REDIRECT_CHECKOUT_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateRedirectCheckoutInput,
  type EscalateRedirectCheckoutOutput,
};
