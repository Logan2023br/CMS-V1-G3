...
function formatStickyNoteContent(
  fields: StickyNoteFields,
  ticketUrl: string
): string {
...
async function escalateStickyIssueHandler(
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatStickyNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_sticky_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_sticky_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateStickyIssueHandler, formatStickyNoteContent };
