...
import { escalateAnimationBrokenIssueHandler } from "@/mcp/tools/escalate_animation_broken_issue/handler.js";
import {
  ESCALATE_ANIMATION_BROKEN_INPUT_SHAPE,
  ESCALATE_ANIMATION_BROKEN_OUTPUT_SHAPE,
} from "@/mcp/tools/escalate_animation_broken_issue/shapes.js";

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type {
  EscalateAnimationBrokenInput,
  EscalateAnimationBrokenOutput,
} from "@/mcp/tools/escalate_animation_broken_issue/shapes.js";
...
function registerEscalateAnimationBrokenIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_animation_broken_issue",
    {
      title: "Escalate existing animation broken / not working on PageFly page",
      description: `
        Call this tool when the customer reports an animation they ALREADY SET UP in the PageFly editor does not work, does not show, or behaves incorrectly on the live page. Common phrasings:
          - "Animation không hoạt động đúng"
          - "Animation không hoạt động"
          - "Đã set animation nhưng không show"
          - "My animation does not play on live"
          - "Animation set up but nothing happens"

        DO NOT use this tool when the customer wants to CREATE a NEW animation effect (e.g. wants to replicate an effect from a reference website) — use escalate_animation_issue for that.
        ...
        DO NOT call this tool until:
          ...
          2. You have a description of WHICH animation is broken + WHAT the broken behavior is + (if relevant) WHAT customer wants it to do (into issue_description), AND
        ...
        - issue_description (required) — Detailed English paraphrase. Name WHICH animation + WHAT the broken behavior is + (if relevant) intended behavior. Example: "Hover animation on product cards set up in editor but does not play on live page."
        - editor_link (required) — PageFly editor URL of the page where the animation is broken.
        - screenshot_urls (optional array) — URLs the user pasted showing the broken animation.
        ...
        STEP 1 — ACKNOWLEDGE. There is no customer-side self-help; the technical team must inspect the animation JS / element setup. Reply:
        "Issue về animation không hoạt động cần team kỹ thuật kiểm tra trực tiếp trên store. Mình sẽ chuyển ticket sang team để các bạn xử lý. Trước đó, cho mình xin thêm vài thông tin nhé."

        STEP 2 — Collect:
        a) Editor link of the affected page. Ask: "Bạn gửi mình link editor của trang có animation đang lỗi nhé."
        b) Detailed description. Ask: "Bạn mô tả rõ hơn giúp mình: animation gì đang bị (hover, scroll, click, fade-in, ...) — hành vi hiện tại như thế nào và bạn muốn nó hoạt động ra sao?"
        c) Visual evidence (OPTIONAL but helpful): "Nếu được, bạn gửi mình ảnh chụp hoặc video ngắn cho thấy animation đang lỗi — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        ...
        a) Call escalate_animation_broken_issue with: issue_description (English, detailed), editor_link, publish_status, user_exited_editor=true. If user pasted screenshot URLs include them in screenshot_urls. If user attached files in chat set customer_attached_files=true. ALWAYS include customer_last_message_text.
      ...
      `,
      inputSchema: ESCALATE_ANIMATION_BROKEN_INPUT_SHAPE,
      outputSchema: ESCALATE_ANIMATION_BROKEN_OUTPUT_SHAPE,
    },
    async (input: EscalateAnimationBrokenInput) => {
      const output: EscalateAnimationBrokenOutput = await escalateAnimationBrokenIssueHandler(input);
      return {
        content: [
          {
            type: "text",
...
export { registerEscalateAnimationBrokenIssueTool };
