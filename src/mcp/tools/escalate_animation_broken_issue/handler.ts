...
interface AnimationBrokenNoteFields {
...
function formatAnimationBrokenNoteContent(
  fields: AnimationBrokenNoteFields,
  ticketUrl: string
): string {
...
async function escalateAnimationBrokenIssueHandler(
  input: EscalateAnimationBrokenInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateAnimationBrokenOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateAnimationBrokenOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatAnimationBrokenNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_animation_broken_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_animation_broken_issue] match: posted=false error=${noteResult.error}`
    );
  }
