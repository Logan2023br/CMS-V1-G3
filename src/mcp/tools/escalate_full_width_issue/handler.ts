...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the page/section showing the constrained width vs the desired full-width result (paste link or attach the file directly in chat)",
  publish_status:
...
function formatFullWidthNoteContent(
  fields: FullWidthNoteFields,
  ticketUrl: string
): string {
...
async function escalateFullWidthIssueHandler(
  input: EscalateFullWidthInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateFullWidthOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatFullWidthNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_full_width_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_full_width_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateFullWidthIssueHandler, formatFullWidthNoteContent };
