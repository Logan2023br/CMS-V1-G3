...
const ESCALATE_FULL_WIDTH_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST include: (a) scope — entire page or a specific section, (b) current behavior (page/section constrained by theme container, has side margins, not edge-to-edge), (c) what the customer wants (full viewport width / full-bleed). Example: 'Customer wants the entire PageFly page rendered at full viewport width; currently theme container constrains it with side margins.', 'Hero section needs to be full-bleed edge-to-edge; currently has 80px side padding from theme.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the constrained-width current state and (if possible) the desired full-width target. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true."
    ),
...
type EscalateFullWidthInput = z.infer<typeof ESCALATE_FULL_WIDTH_INPUT_SHAPE>;
...
const ESCALATE_FULL_WIDTH_OUTPUT_SHAPE = z.object({
...
type EscalateFullWidthOutput = z.infer<typeof ESCALATE_FULL_WIDTH_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateFullWidthInput,
  type EscalateFullWidthOutput,
};
