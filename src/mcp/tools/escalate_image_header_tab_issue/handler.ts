...
interface ImageHeaderTabNoteFields {
...
function formatImageHeaderTabNoteContent(
  fields: ImageHeaderTabNoteFields,
  ticketUrl: string
): string {
...
async function escalateImageHeaderTabIssueHandler(
  input: EscalateImageHeaderTabInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateImageHeaderTabOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateImageHeaderTabOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatImageHeaderTabNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_image_header_tab_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_image_header_tab_issue] match: posted=false error=${noteResult.error}`
    );
  }
