...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot showing where the compare price should appear or where it shows the wrong value (paste link or attach the file directly in chat)",
  publish_status:
...
interface ComparePriceNoteFields {
...
function formatComparePriceNoteContent(
  fields: ComparePriceNoteFields,
  ticketUrl: string
): string {
...
async function escalateComparePriceIssueHandler(
  input: EscalateComparePriceInput,
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatComparePriceNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_compare_price_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_compare_price_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateComparePriceIssueHandler, formatComparePriceNoteContent };
