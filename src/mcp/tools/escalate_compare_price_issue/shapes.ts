...
const ESCALATE_COMPARE_PRICE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST distinguish between (a) compare price not displaying at all and (b) compare price displaying wrong value, and name WHICH product/page if relevant. Example: 'Compare price not visible on product page; product variant has compare_at_price set in Shopify.', 'Compare price displays $0 instead of original price on product XYZ.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the broken compare price location (annotated if possible). Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true so TS can locate the element."
    ),
...
type EscalateComparePriceInput = z.infer<typeof ESCALATE_COMPARE_PRICE_INPUT_SHAPE>;
...
const ESCALATE_COMPARE_PRICE_OUTPUT_SHAPE = z.object({
...
type EscalateComparePriceOutput = z.infer<typeof ESCALATE_COMPARE_PRICE_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateComparePriceInput,
  type EscalateComparePriceOutput,
};
