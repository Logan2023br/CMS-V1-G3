...
const ESCALATE_HIDDEN_SOLDOUT_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the request, ALWAYS IN ENGLISH. Mention that the customer has agreed to the free custom-code solution. Example: 'Customer wants to hide sold-out variants on PageFly page; agreed to free custom-code solution.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the variants they want to hide (screenshot of product page with sold-out variants visible). Omit if the user attached files directly in chat."
    ),
  ...
  customer_attached_files: z
    ...
    .optional()
    .describe(
      "Set TRUE if the user attached files directly in the Crisp chat (image upload) instead of pasting links."
    ),
...
type EscalateHiddenSoldoutInput = z.infer<typeof ESCALATE_HIDDEN_SOLDOUT_INPUT_SHAPE>;
...
const ESCALATE_HIDDEN_SOLDOUT_OUTPUT_SHAPE = z.object({
...
type EscalateHiddenSoldoutOutput = z.infer<typeof ESCALATE_HIDDEN_SOLDOUT_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateHiddenSoldoutInput,
  type EscalateHiddenSoldoutOutput,
};
