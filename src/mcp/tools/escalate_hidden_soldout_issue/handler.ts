...
interface HiddenSoldoutNoteFields {
...
function formatHiddenSoldoutNoteContent(
  fields: HiddenSoldoutNoteFields,
  ticketUrl: string
): string {
...
async function escalateHiddenSoldoutIssueHandler(
  input: EscalateHiddenSoldoutInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateHiddenSoldoutOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateHiddenSoldoutOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatHiddenSoldoutNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_hidden_soldout_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_hidden_soldout_issue] match: posted=false error=${noteResult.error}`
    );
  }
