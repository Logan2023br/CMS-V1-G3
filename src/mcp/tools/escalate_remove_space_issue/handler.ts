...
interface RemoveSpaceNoteFields {
...
function formatRemoveSpaceNoteContent(
  fields: RemoveSpaceNoteFields,
  ticketUrl: string
): string {
...
async function escalateRemoveSpaceIssueHandler(
  input: EscalateRemoveSpaceInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateRemoveSpaceOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatRemoveSpaceNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_remove_space_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_remove_space_issue] match: posted=false error=${noteResult.error}`
    );
  }
