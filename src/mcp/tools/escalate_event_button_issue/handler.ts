...
interface EventButtonNoteFields {
...
function formatEventButtonNoteContent(
  fields: EventButtonNoteFields,
  ticketUrl: string
): string {
...
async function escalateEventButtonIssueHandler(
  input: EscalateEventButtonInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateEventButtonOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatEventButtonNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_event_button_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_event_button_issue] match: posted=false error=${noteResult.error}`
    );
  }
