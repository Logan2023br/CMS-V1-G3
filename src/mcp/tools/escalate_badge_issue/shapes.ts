...
const ESCALATE_BADGE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST name the badge variant (Sale / New / Sold-out / Custom) AND classify the symptom: not showing, showing wrong value/text, live correct but preview/liquid wrong, badge shows when product has no compare price, etc. Example: 'Sale badge not showing on product list page; products have compare_at_price set.', 'Sale badge shows wrong percentage value on live (says 50% but should be 30%).', 'Badge displays correctly on live but shows wrong on preview/liquid render.', 'Sale badge appears on products without compare_at_price set.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the broken badge location. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true so TS can confirm symptom and location."
    ),
...
type EscalateBadgeInput = z.infer<typeof ESCALATE_BADGE_INPUT_SHAPE>;
...
const ESCALATE_BADGE_OUTPUT_SHAPE = z.object({
...
type EscalateBadgeOutput = z.infer<typeof ESCALATE_BADGE_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateBadgeInput,
  type EscalateBadgeOutput,
};
