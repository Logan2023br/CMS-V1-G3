...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the broken badge location (paste link or attach the file directly in chat)",
  publish_status:
...
function formatBadgeNoteContent(
  fields: BadgeNoteFields,
  ticketUrl: string
): string {
...
async function escalateBadgeIssueHandler(
  input: EscalateBadgeInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateBadgeOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatBadgeNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_badge_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_badge_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateBadgeIssueHandler, formatBadgeNoteContent };
