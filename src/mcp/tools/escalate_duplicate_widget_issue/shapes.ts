...
const ESCALATE_DUPLICATE_WIDGET_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's detailed English paraphrase. Identify WHICH widget/app is duplicated (app name if customer named it: app review, app bundle, 3rd-party widget, etc.) and WHERE it appears doubled. Examples: 'Judge.me review widget appears twice on product page.', 'App bundle widget duplicated on PageFly section.', 'Customer's added 3rd-party widget shows duplicated on live page.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the duplicated widget (screenshot of the live page with two copies visible). Omit if the user attached files directly in chat."
    ),
...
type EscalateDuplicateWidgetInput = z.infer<typeof ESCALATE_DUPLICATE_WIDGET_INPUT_SHAPE>;
...
const ESCALATE_DUPLICATE_WIDGET_OUTPUT_SHAPE = z.object({
...
type EscalateDuplicateWidgetOutput = z.infer<typeof ESCALATE_DUPLICATE_WIDGET_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateDuplicateWidgetInput,
  type EscalateDuplicateWidgetOutput,
};
