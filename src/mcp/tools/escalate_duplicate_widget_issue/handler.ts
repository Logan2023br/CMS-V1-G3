...
interface DuplicateWidgetNoteFields {
...
function formatDuplicateWidgetNoteContent(
  fields: DuplicateWidgetNoteFields,
  ticketUrl: string
): string {
...
async function escalateDuplicateWidgetIssueHandler(
  input: EscalateDuplicateWidgetInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateDuplicateWidgetOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateDuplicateWidgetOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatDuplicateWidgetNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_duplicate_widget_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_duplicate_widget_issue] match: posted=false error=${noteResult.error}`
    );
  }
