...
const ESCALATE_WHITE_PAGE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. Note WHAT the customer sees on live (blank/white screen, possibly 404). Example: 'Live page renders blank white; content visible in editor.', 'Page is a white screen on live storefront — editor preview looks fine.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the white/blank page on live (or browser-tab 404 / dev-tools console). Omit if the user attached files directly in chat."
    ),
...
type EscalateWhitePageInput = z.infer<typeof ESCALATE_WHITE_PAGE_INPUT_SHAPE>;
...
const ESCALATE_WHITE_PAGE_OUTPUT_SHAPE = z.object({
...
type EscalateWhitePageOutput = z.infer<typeof ESCALATE_WHITE_PAGE_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateWhitePageInput,
  type EscalateWhitePageOutput,
};
