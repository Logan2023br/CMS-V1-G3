...
function formatWhitePageNoteContent(
  fields: WhitePageNoteFields,
  ticketUrl: string
): string {
...
async function escalateWhitePageIssueHandler(
  input: EscalateWhitePageInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateWhitePageOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatWhitePageNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_white_page_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_white_page_issue] match: posted=false error=${noteResult.error}`
    );
  }
