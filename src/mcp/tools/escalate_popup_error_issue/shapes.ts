...
const ESCALATE_POPUP_ERROR_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's detailed English paraphrase of the popup issue. Note WHAT is broken (e.g. popup does not open, popup shows only in editor, popup auto-closes). Example: 'Popup opens in editor but does not appear on live page or preview.', 'Popup trigger does not fire on live storefront.'"
    ),
  ...
  editor_link: z
    ...
    .url()
    .describe(
      "The PageFly editor URL of the page that contains the broken popup. Take what the user pasted. No placeholders."
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the popup issue (screenshot of editor where popup is set up, screen recording showing live page with no popup). Omit if the user attached files directly in chat."
    ),
...
type EscalatePopupErrorInput = z.infer<typeof ESCALATE_POPUP_ERROR_INPUT_SHAPE>;
...
const ESCALATE_POPUP_ERROR_OUTPUT_SHAPE = z.object({
...
type EscalatePopupErrorOutput = z.infer<typeof ESCALATE_POPUP_ERROR_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalatePopupErrorInput,
  type EscalatePopupErrorOutput,
};
