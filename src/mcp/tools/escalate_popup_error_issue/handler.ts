...
interface PopupErrorNoteFields {
...
function formatPopupErrorNoteContent(
  fields: PopupErrorNoteFields,
  ticketUrl: string
): string {
...
async function escalatePopupErrorIssueHandler(
  input: EscalatePopupErrorInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalatePopupErrorOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatPopupErrorNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_popup_error_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_popup_error_issue] match: posted=false error=${noteResult.error}`
    );
  }
