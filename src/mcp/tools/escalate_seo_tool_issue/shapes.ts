...
const ESCALATE_SEO_TOOL_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST name (a) which SEO tool check / score is wrong, (b) what value the tool shows vs. what the customer expects, and (c) what fix the customer needs. Example: 'PageFly SEO tool flags meta description as missing although it is set in editor; needs fix.', 'SEO tool reports duplicate H1 incorrectly on product page; customer wants the warning resolved.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the wrong SEO tool report / issue. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true."
    ),
  ...
  user_consented_to_publish: z
    .boolean()
    .describe(
      "MUST be true. The user has explicitly agreed that the technical team may publish the page after fixing. TS team WILL publish (no save-only option for SEO fixes — they only take effect on published pages). Ask first if you have not."
    ),
...
type EscalateSeoToolInput = z.infer<typeof ESCALATE_SEO_TOOL_INPUT_SHAPE>;
...
const ESCALATE_SEO_TOOL_OUTPUT_SHAPE = z.object({
...
type EscalateSeoToolOutput = z.infer<typeof ESCALATE_SEO_TOOL_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateSeoToolInput,
  type EscalateSeoToolOutput,
};
