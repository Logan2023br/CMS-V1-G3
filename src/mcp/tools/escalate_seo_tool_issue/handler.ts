...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the SEO tool report / issue (paste link or attach the file directly in chat)",
...
function formatSeoToolNoteContent(
  fields: SeoToolNoteFields,
  ticketUrl: string
): string {
...
async function escalateSeoToolIssueHandler(
  input: EscalateSeoToolInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateSeoToolOutput> {
  ...
  if (missing.length > 0) {
    ...
    return {
      ...
      note_posted: false,
      note_post_error:
        "Not ready for escalation — Hugo MUST collect a real editor link, screenshot evidence, AND explicit user consent to publish. SEO fixes require publish to take effect.",
    };
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatSeoToolNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_seo_tool_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_seo_tool_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateSeoToolIssueHandler, formatSeoToolNoteContent };
