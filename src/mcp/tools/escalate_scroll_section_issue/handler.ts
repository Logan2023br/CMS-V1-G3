...
interface ScrollSectionNoteFields {
...
function formatScrollSectionNoteContent(
  fields: ScrollSectionNoteFields,
  ticketUrl: string
): string {
...
async function escalateScrollSectionIssueHandler(
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatScrollSectionNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_scroll_section_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_scroll_section_issue] match: posted=false error=${noteResult.error}`
    );
  }
