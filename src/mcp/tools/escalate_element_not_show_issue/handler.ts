...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the page showing which element is missing and on which viewport (paste link or attach the file directly in chat)",
  publish_status:
...
interface ElementNotShowNoteFields {
...
function formatElementNotShowNoteContent(
  fields: ElementNotShowNoteFields,
  ticketUrl: string
): string {
...
async function escalateElementNotShowIssueHandler(
  input: EscalateElementNotShowInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateElementNotShowOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateElementNotShowOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatElementNotShowNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_element_not_show_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_element_not_show_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateElementNotShowIssueHandler, formatElementNotShowNoteContent };
