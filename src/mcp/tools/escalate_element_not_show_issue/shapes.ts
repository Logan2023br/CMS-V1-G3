...
const ESCALATE_ELEMENT_NOT_SHOW_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. Specify WHICH element is not visible + WHICH viewport (mobile / desktop / both). Example: 'Hero image not visible on mobile viewport; renders correctly on desktop.', 'CTA button missing on mobile only.', 'Slideshow not displaying on mobile.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the missing element. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true so TS can confirm which element is missing and on which viewport."
    ),
...
type EscalateElementNotShowInput = z.infer<typeof ESCALATE_ELEMENT_NOT_SHOW_INPUT_SHAPE>;
...
const ESCALATE_ELEMENT_NOT_SHOW_OUTPUT_SHAPE = z.object({
...
type EscalateElementNotShowOutput = z.infer<typeof ESCALATE_ELEMENT_NOT_SHOW_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateElementNotShowInput,
  type EscalateElementNotShowOutput,
};
