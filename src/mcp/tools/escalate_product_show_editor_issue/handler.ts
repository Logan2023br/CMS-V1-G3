...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the editor showing the missing product / empty list (paste link or attach the file directly in chat)",
...
interface ProductShowEditorNoteFields {
...
function formatProductShowEditorNoteContent(
  fields: ProductShowEditorNoteFields,
  ticketUrl: string
): string {
...
async function escalateProductShowEditorIssueHandler(
  input: EscalateProductShowEditorInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateProductShowEditorOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateProductShowEditorOutput;
  }
  ...
  if (missing.length > 0) {
    ...
    return {
      ...
      note_posted: false,
      note_post_error:
        "Not ready for escalation — Hugo MUST collect a real editor link, screenshot evidence, AND explicit user consent to publish.",
    };
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatProductShowEditorNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_product_show_editor_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_product_show_editor_issue] match: posted=false error=${noteResult.error}`
    );
  }
