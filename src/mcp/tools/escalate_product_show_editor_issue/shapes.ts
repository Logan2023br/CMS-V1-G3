...
const ESCALATE_PRODUCT_SHOW_EDITOR_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST name WHICH product / product list / collection is failing to appear in the PageFly editor (where it should normally be selectable or rendered). Example: 'Product XYZ does not appear in the PageFly editor product picker.', 'Collection feed in PageFly editor is empty although collection contains 20 products in Shopify.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing the editor with the missing product / empty product list. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true."
    ),
...
type EscalateProductShowEditorInput = z.infer<typeof ESCALATE_PRODUCT_SHOW_EDITOR_INPUT_SHAPE>;
...
const ESCALATE_PRODUCT_SHOW_EDITOR_OUTPUT_SHAPE = z.object({
...
type EscalateProductShowEditorOutput = z.infer<typeof ESCALATE_PRODUCT_SHOW_EDITOR_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateProductShowEditorInput,
  type EscalateProductShowEditorOutput,
};
