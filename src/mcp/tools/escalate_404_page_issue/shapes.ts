...
const ESCALATE_404_PAGE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the 404 issue, ALWAYS IN ENGLISH. Example: 'PageFly page returns 404 on live storefront — saved fine in editor.', 'Live URL gives 404 after publish.'"
    ),
  ...
  live_preview_url: z
    ...
    .url()
    .describe(
      "The live URL where the page returns 404 (e.g. https://store.myshopify.com/pages/abc). Required so the technical team can reproduce the 404. No placeholders."
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the 404 page (screenshot of the live storefront with the 404). Omit if the user attached files directly in chat."
    ),
...
type Escalate404PageInput = z.infer<typeof ESCALATE_404_PAGE_INPUT_SHAPE>;
...
const ESCALATE_404_PAGE_OUTPUT_SHAPE = z.object({
  issue_summary: z.string(),

  is_ready_for_escalation: z
    .boolean()
    .describe(
      "True iff editor_link AND live_preview_url are provided AND user_consented_to_publish === true AND store access is granted AND the customer has exited the editor. Screenshot is optional."
    ),

  missing_info: z
    .array(z.string())
    .describe(
      "List of fields still missing. Possible values: 'editor_link', 'live_preview_url', 'user_consented_to_publish', 'store_access', 'editor_exit'."
    ),
...
type Escalate404PageOutput = z.infer<typeof ESCALATE_404_PAGE_OUTPUT_SHAPE>;
...
export {
  ...
  type Escalate404PageInput,
  type Escalate404PageOutput,
};
