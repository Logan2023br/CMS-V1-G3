...
import { escalate404PageIssueHandler } from "@/mcp/tools/escalate_404_page_issue/handler.js";
import {
  ESCALATE_404_PAGE_INPUT_SHAPE,
  ESCALATE_404_PAGE_OUTPUT_SHAPE,
...
function registerEscalate404PageIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_404_page_issue",
    {
      title: "Escalate PageFly page returning 404 on live storefront",
      description: `
        Call this tool when the user reports a PageFly page returns 404 / cannot be viewed on the live storefront. Common phrasings:
          - "Page bị 404 khi xem trên live"
          - "Page không xem được trên live"
          - "Page bị lỗi 404 trên storefront"
          - "404 page when accessing my PageFly page"
          - Any "page cannot be viewed on live" / 404 complaint.
        ...
        DO NOT call this tool until:
          ...
          2. You have a real live URL the user actually pasted (where the 404 happens), AND
        ...
        - issue_description (required) — One-line English paraphrase. Example: "PageFly page returns 404 on live storefront."
        - editor_link (required) — PageFly editor URL the user pasted.
        - live_preview_url (required) — Live URL where the 404 happens (e.g. https://store.myshopify.com/pages/...).
        - screenshot_urls (optional array) — URLs the user pasted showing the 404 page.
        ...
        STEP 1 — User reports a 404 issue. There is no customer-side self-help; the technical team must investigate why the page is not resolving. Acknowledge and proceed:
        "Mình sẽ chuyển ticket sang team kỹ thuật để các bạn kiểm tra trang bị 404 cho bạn. Trước đó, cho mình xin thêm vài thông tin nhé."

        STEP 2 — Collect:
        a) Editor link of the affected page. Ask: "Bạn gửi mình link editor của trang đang bị 404 nhé."
        b) Live URL where the 404 happens. Ask: "Và link live của trang đó (link bạn truy cập bị 404) nữa nhé."
        c) Evidence (OPTIONAL but helpful): "Nếu được, bạn gửi mình ảnh chụp trang 404 đang hiện ra — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        d) Publish consent: "Khi team kỹ thuật fix xong, mình publish luôn trang lên cho bạn nhé? (cần publish để áp dụng fix)"

        STEP 3 — Have editor_link + live_preview_url + user said YES to publish. BEFORE calling the tool, ask the customer to EXIT the editor and WAIT for explicit confirmation:
      ...
      `,
      inputSchema: ESCALATE_404_PAGE_INPUT_SHAPE,
      outputSchema: ESCALATE_404_PAGE_OUTPUT_SHAPE,
    },
    async (input: Escalate404PageInput) => {
      const output: Escalate404PageOutput = await escalate404PageIssueHandler(input);
      return {
        content: [
          {
            type: "text",
...
export { registerEscalate404PageIssueTool };
