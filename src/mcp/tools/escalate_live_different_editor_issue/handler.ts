...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  live_preview_url: "the live URL where the issue is visible",
  publish_status:
...
interface LiveDifferentEditorNoteFields {
...
function formatLiveDifferentEditorNoteContent(
  fields: LiveDifferentEditorNoteFields,
  ticketUrl: string
): string {
...
async function escalateLiveDifferentEditorIssueHandler(
  input: EscalateLiveDifferentEditorInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateLiveDifferentEditorOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateLiveDifferentEditorOutput;
  }
  ...
  if (missing.length > 0) {
    ...
    return {
      ...
      note_posted: false,
      note_post_error:
        "Not ready for escalation — Hugo MUST collect a real editor link, real live URL, AND the publish_status answer. Do NOT fabricate URLs or status values.",
    };
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatLiveDifferentEditorNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_live_different_editor_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_live_different_editor_issue] match: posted=false error=${noteResult.error}`
    );
  }
