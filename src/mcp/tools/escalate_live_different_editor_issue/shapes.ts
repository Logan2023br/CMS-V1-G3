...
const ESCALATE_LIVE_DIFFERENT_EDITOR_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's detailed English paraphrase. Capture WHAT the customer wants adjusted on the page or WHAT the visual mismatch is between editor and live (e.g. element not aligned, image cropped wrong, layout broken on live). Example: 'Live storefront not aligning product images correctly; editor preview looks fine — customer wants alignment fixed.', 'Customer wants the hero image enlarged and the CTA button moved to the right; live and editor differ.'"
    ),
  ...
  live_preview_url: z
    ...
    .url()
    .describe(
      "The live URL where the issue is visible (so TS can compare live vs. editor). Take what the user pasted (e.g. https://store.myshopify.com/pages/...). No placeholders."
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the issue (screenshot pointing at the mismatch or the UI the customer wants adjusted). Omit if the user attached files directly in chat."
    ),
...
type EscalateLiveDifferentEditorInput = z.infer<typeof ESCALATE_LIVE_DIFFERENT_EDITOR_INPUT_SHAPE>;
...
const ESCALATE_LIVE_DIFFERENT_EDITOR_OUTPUT_SHAPE = z.object({
  issue_summary: z.string(),

  is_ready_for_escalation: z
    .boolean()
    .describe(
      "True iff editor_link AND live_preview_url are provided AND publish_status is set AND store access is granted AND the customer has exited the editor. Screenshot is optional."
    ),

  missing_info: z
    .array(z.string())
    .describe(
      "List of fields still missing. Possible values: 'editor_link', 'live_preview_url', 'publish_status', 'store_access', 'editor_exit'."
    ),
...
type EscalateLiveDifferentEditorOutput = z.infer<typeof ESCALATE_LIVE_DIFFERENT_EDITOR_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateLiveDifferentEditorInput,
  type EscalateLiveDifferentEditorOutput,
};
