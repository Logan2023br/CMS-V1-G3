...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the section to reshape and (if possible) the reference layout you want (paste link or attach the file directly in chat)",
  publish_status:
...
interface EditLayoutNoteFields {
...
function formatEditLayoutNoteContent(
  fields: EditLayoutNoteFields,
  ticketUrl: string
): string {
...
async function escalateEditLayoutIssueHandler(
  input: EscalateEditLayoutInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateEditLayoutOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatEditLayoutNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_edit_layout_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_edit_layout_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateEditLayoutIssueHandler, formatEditLayoutNoteContent };
