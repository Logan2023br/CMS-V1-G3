...
const ESCALATE_EDIT_LAYOUT_BUNDLE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the styling request, ALWAYS IN ENGLISH. MUST include: (a) which 3rd-party app widget is being restyled (bundle, review, upsell, ...), (b) current appearance, (c) desired appearance (colors, fonts, button styles, sizes). Example: 'Customer wants the bundle app Add-to-Cart button restyled: background #FF6B00 (orange), text white, larger padding.', 'Restyle review widget stars to gold and review count to gray italic.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot URLs the user pasted showing both the current widget appearance and the desired result (annotated mockup, reference design). Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true."
    ),
  ...
  user_consented_to_publish: z
    .boolean()
    .describe(
      "MUST be true. The user has explicitly agreed that the technical team may publish the page after applying the styling fix. TS team WILL publish (no save-only option — restyle changes need to be visible on live). Ask first if you have not."
    ),
...
type EscalateEditLayoutBundleInput = z.infer<typeof ESCALATE_EDIT_LAYOUT_BUNDLE_INPUT_SHAPE>;
...
const ESCALATE_EDIT_LAYOUT_BUNDLE_OUTPUT_SHAPE = z.object({
...
type EscalateEditLayoutBundleOutput = z.infer<typeof ESCALATE_EDIT_LAYOUT_BUNDLE_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateEditLayoutBundleInput,
  type EscalateEditLayoutBundleOutput,
};
