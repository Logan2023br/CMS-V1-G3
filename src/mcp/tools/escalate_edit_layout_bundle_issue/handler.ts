...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot of the current widget appearance and the desired styling (paste link or attach the file directly in chat)",
  user_consented_to_publish:
    "your permission to publish the page after the technical team applies the styling",
};
...
interface EditLayoutBundleNoteFields {
...
function formatEditLayoutBundleNoteContent(
  fields: EditLayoutBundleNoteFields,
  ticketUrl: string
): string {
...
async function escalateEditLayoutBundleIssueHandler(
  input: EscalateEditLayoutBundleInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateEditLayoutBundleOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateEditLayoutBundleOutput;
  }
  ...
  if (missing.length > 0) {
    ...
    return {
      ...
      note_posted: false,
      note_post_error:
        "Not ready for escalation — Hugo MUST collect a real editor link, screenshot evidence, AND explicit user consent to publish. Restyling needs publish to be visible on live.",
    };
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatEditLayoutBundleNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_edit_layout_bundle_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_edit_layout_bundle_issue] match: posted=false error=${noteResult.error}`
    );
  }
