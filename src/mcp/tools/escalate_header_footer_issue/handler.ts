...
interface HeaderFooterNoteFields {
...
function formatHeaderFooterNoteContent(
  fields: HeaderFooterNoteFields,
  ticketUrl: string
): string {
...
async function escalateHeaderFooterIssueHandler(
  ...
  if (noteResult.posted) {
    console.log(
      `[escalate_header_footer_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_header_footer_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateHeaderFooterIssueHandler, formatHeaderFooterNoteContent };
