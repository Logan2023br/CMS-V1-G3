...
const ESCALATE_HEADER_FOOTER_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST classify the variant: (a) Hide-toggle on but header/footer still shows, (b) Hide-toggle off but header/footer is missing on live, (c) Header/footer missing without any toggle change, (d) Customer wants to hide ONLY header or ONLY footer (not both). Example: 'Customer toggled hide-header in PageFly page settings but live page still renders the theme header.', 'Header/footer missing on live page despite hide-toggle not being enabled.', 'Customer wants to hide only header while keeping footer; needs guidance/fix.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "OPTIONAL — screenshot URLs the user pasted showing the broken header/footer behavior. Include if customer provides; do not block escalation on it."
    ),
...
type EscalateHeaderFooterInput = z.infer<typeof ESCALATE_HEADER_FOOTER_INPUT_SHAPE>;
...
const ESCALATE_HEADER_FOOTER_OUTPUT_SHAPE = z.object({
...
type EscalateHeaderFooterOutput = z.infer<typeof ESCALATE_HEADER_FOOTER_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateHeaderFooterInput,
  type EscalateHeaderFooterOutput,
};
