...
interface OverrideSectionThemeNoteFields {
...
function formatOverrideSectionThemeNoteContent(
  fields: OverrideSectionThemeNoteFields,
  ticketUrl: string
): string {
...
async function escalateOverrideSectionThemeIssueHandler(
  input: EscalateOverrideSectionThemeInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateOverrideSectionThemeOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateOverrideSectionThemeOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatOverrideSectionThemeNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_override_section_theme_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_override_section_theme_issue] match: posted=false error=${noteResult.error}`
    );
  }
