...
const ESCALATE_OVERRIDE_SECTION_THEME_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. Example: 'Theme section showing on PageFly page instead of (or alongside) PageFly section after publish.', 'PageFly section not visible — theme section displayed in its place.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the issue (screenshot of the live page with theme section visible instead of PageFly section). Omit if the user attached files directly in chat."
    ),
...
type EscalateOverrideSectionThemeInput = z.infer<typeof ESCALATE_OVERRIDE_SECTION_THEME_INPUT_SHAPE>;
...
const ESCALATE_OVERRIDE_SECTION_THEME_OUTPUT_SHAPE = z.object({
...
type EscalateOverrideSectionThemeOutput = z.infer<typeof ESCALATE_OVERRIDE_SECTION_THEME_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateOverrideSectionThemeInput,
  type EscalateOverrideSectionThemeOutput,
};
