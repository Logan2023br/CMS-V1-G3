...
import { escalateAppErrorPositionIssueHandler } from "@/mcp/tools/escalate_app_error_position_issue/handler.js";
import {
  ESCALATE_APP_ERROR_POSITION_INPUT_SHAPE,
  ESCALATE_APP_ERROR_POSITION_OUTPUT_SHAPE,
} from "@/mcp/tools/escalate_app_error_position_issue/shapes.js";

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type {
  EscalateAppErrorPositionInput,
  EscalateAppErrorPositionOutput,
} from "@/mcp/tools/escalate_app_error_position_issue/shapes.js";
...
function registerEscalateAppErrorPositionIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_app_error_position_issue",
    {
      title: "Escalate app/widget showing in wrong position on PageFly page",
      description: `
        Call this tool when the customer reports an app, widget, or 3rd-party block shows in the WRONG POSITION on their PageFly page (right element, wrong place). Common phrasings:
          - "Giao diện của bundle show sai vị trí"
          - "App show sai vị trí"
          - "App 3rd show sai vị trí"
          - "App tôi add vào sao show sai vị trí"
          - "My added app shows in the wrong place"
          - "Widget appears in the footer but should be in the product section"

        DO NOT use this tool when:
          - The app shows TWICE (duplicated) → use escalate_duplicate_widget_issue.
        ...
        DO NOT call this tool until:
          ...
          2. You have a description of WHICH app + WHERE it shows now + WHERE customer wants it (into issue_description), AND
        ...
        - issue_description (required) — Detailed English paraphrase. Name the app + describe current vs. desired position. Example: "Judge.me review widget shows at top of product page instead of below description.", "Bundle app appears in footer but customer wants it inside the product section."
        - editor_link (required) — PageFly editor URL of the affected page.
        - screenshot_urls (optional array) — URLs the user pasted showing where the app appears now / where it should appear (annotated screenshot).
        ...
        STEP 1 — ACKNOWLEDGE. There is no customer-side self-help; the technical team must move/anchor the app block. Reply:
        "Issue về app hiển thị sai vị trí này cần team kỹ thuật kiểm tra trực tiếp trên store. Mình sẽ chuyển ticket sang team để các bạn xử lý. Trước đó, cho mình xin thêm vài thông tin nhé."

        STEP 2 — Collect:
        a) Editor link of the affected page. Ask: "Bạn gửi mình link editor của trang đang gặp lỗi nhé."
        b) Detailed description — WHICH app + WHERE nó đang hiện + WHERE bạn muốn nó hiện. Ask: "Bạn cho mình biết app nào đang hiển thị sai vị trí và vị trí đúng mà bạn muốn nó hiển thị nhé."
        c) Visual evidence (OPTIONAL but helpful): "Nếu được, bạn gửi mình ảnh chụp đánh dấu vị trí hiện tại + vị trí mong muốn — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        ...
        a) Call escalate_app_error_position_issue with: issue_description (English, name app + positions), editor_link, user_consented_to_publish=true, user_exited_editor=true. If user pasted screenshot URLs include them in screenshot_urls. If user attached files in chat set customer_attached_files=true. ALWAYS include customer_last_message_text.
      ...
      `,
      inputSchema: ESCALATE_APP_ERROR_POSITION_INPUT_SHAPE,
      outputSchema: ESCALATE_APP_ERROR_POSITION_OUTPUT_SHAPE,
    },
    async (input: EscalateAppErrorPositionInput) => {
      const output: EscalateAppErrorPositionOutput = await escalateAppErrorPositionIssueHandler(input);
      return {
        content: [
          {
            type: "text",
...
export { registerEscalateAppErrorPositionIssueTool };
