...
interface AppErrorPositionNoteFields {
...
function formatAppErrorPositionNoteContent(
  fields: AppErrorPositionNoteFields,
  ticketUrl: string
): string {
...
async function escalateAppErrorPositionIssueHandler(
  input: EscalateAppErrorPositionInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateAppErrorPositionOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateAppErrorPositionOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatAppErrorPositionNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_app_error_position_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_app_error_position_issue] match: posted=false error=${noteResult.error}`
    );
  }
