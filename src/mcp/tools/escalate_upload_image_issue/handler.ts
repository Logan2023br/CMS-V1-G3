...
interface UploadImageNoteFields {
...
function formatUploadImageNoteContent(
  fields: UploadImageNoteFields,
  ticketUrl: string
): string {
...
async function escalateUploadImageIssueHandler(
  input: EscalateUploadImageInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateUploadImageOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatUploadImageNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_upload_image_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_upload_image_issue] match: posted=false error=${noteResult.error}`
    );
  }
