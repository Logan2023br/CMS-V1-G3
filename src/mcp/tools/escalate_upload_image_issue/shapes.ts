...
const ESCALATE_UPLOAD_IMAGE_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the upload-image issue, ALWAYS IN ENGLISH. Mention the customer confirmed they are the store owner (so the permission self-help is ruled out). Example: 'Customer is store owner — cannot upload image in PageFly editor, sees error message.'"
    ),
  ...
  editor_link: z
    ...
    .url()
    .describe(
      "The PageFly editor URL where the upload error occurs. Take what the user pasted. No placeholders."
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the upload error (screenshot of the error message in PageFly editor). Omit if the user attached files directly in chat."
    ),
...
type EscalateUploadImageInput = z.infer<typeof ESCALATE_UPLOAD_IMAGE_INPUT_SHAPE>;
...
const ESCALATE_UPLOAD_IMAGE_OUTPUT_SHAPE = z.object({
...
type EscalateUploadImageOutput = z.infer<typeof ESCALATE_UPLOAD_IMAGE_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateUploadImageInput,
  type EscalateUploadImageOutput,
};
