...
const ESCALATE_VARIANT_MEDIA_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line English paraphrase. Example: 'Product media does not switch when customer selects a variant on product page.', 'Variant images stay on default image when switching variants.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the issue (screenshot pointing at the variant selector + product media, or short recording showing the variant switch with no media change). Omit if the user attached files directly in chat."
    ),
...
type EscalateVariantMediaInput = z.infer<typeof ESCALATE_VARIANT_MEDIA_INPUT_SHAPE>;
...
const ESCALATE_VARIANT_MEDIA_OUTPUT_SHAPE = z.object({
...
type EscalateVariantMediaOutput = z.infer<typeof ESCALATE_VARIANT_MEDIA_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateVariantMediaInput,
  type EscalateVariantMediaOutput,
};
