...
interface VariantMediaNoteFields {
...
function formatVariantMediaNoteContent(
  fields: VariantMediaNoteFields,
  ticketUrl: string
): string {
...
async function escalateVariantMediaIssueHandler(
  input: EscalateVariantMediaInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateVariantMediaOutput> {
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatVariantMediaNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_variant_media_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_variant_media_issue] match: posted=false error=${noteResult.error}`
    );
  }
