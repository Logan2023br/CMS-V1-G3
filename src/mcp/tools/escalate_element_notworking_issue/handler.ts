...
interface ElementNotworkingNoteFields {
...
function formatElementNotworkingNoteContent(
  fields: ElementNotworkingNoteFields,
  ticketUrl: string
): string {
...
async function escalateElementNotworkingIssueHandler(
  input: EscalateElementNotworkingInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateElementNotworkingOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateElementNotworkingOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatElementNotworkingNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_element_notworking_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_element_notworking_issue] match: posted=false error=${noteResult.error}`
    );
  }
