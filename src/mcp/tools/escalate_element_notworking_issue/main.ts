...
import { escalateElementNotworkingIssueHandler } from "@/mcp/tools/escalate_element_notworking_issue/handler.js";
import {
  ESCALATE_ELEMENT_NOTWORKING_INPUT_SHAPE,
  ESCALATE_ELEMENT_NOTWORKING_OUTPUT_SHAPE,
} from "@/mcp/tools/escalate_element_notworking_issue/shapes.js";

import type { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import type {
  EscalateElementNotworkingInput,
  EscalateElementNotworkingOutput,
} from "@/mcp/tools/escalate_element_notworking_issue/shapes.js";
...
function registerEscalateElementNotworkingIssueTool(server: McpServer): void {
  server.registerTool(
    "escalate_element_notworking_issue",
    {
      title: "Escalate broken PageFly element (navigation / dropdown / slideshow / pagination)",
      description: `
        Call this tool when the user reports a specific interactive PageFly element does not work — examples include navigation menus, dropdowns, slideshows, pagination, accordions, tabs, modals. Common phrasings:
          - "Navigation không hoạt động"
          - "Dropdown không hoạt động"
          - "Slideshow không hoạt động đúng"
          - "Pagination không show"
          - "Accordion không mở được"
          - "Tab content không switch"
          - "My PageFly <element> is not working"

        For OTHER issues with dedicated tools (sticky / horizontal scroll / scroll-to-section / animation / cart drawer / theme override / 404 / white page / variant A-B / image upload / image missing / etc.), use the matching tool. Use this one ONLY for interactive PageFly element failure not covered elsewhere.
        ...
        DO NOT call this tool until:
          ...
          2. You have a detailed description of WHICH element is broken and WHAT the behavior is (this goes into issue_description), AND
        ...
        - issue_description (required) — Detailed English paraphrase. Name WHICH element + WHAT the broken behavior is. Example: "Navigation menu does not open when clicked on mobile.", "Slideshow auto-advance does not work; arrows are unresponsive."
        - editor_link (required) — PageFly editor URL the user pasted.
        - screenshot_urls (optional array) — URLs the user pasted (screenshot / video) demonstrating the broken behavior.
        ...
        STEP 1 — ACKNOWLEDGE. There is no customer-side self-help; the technical team must inspect the element JS/HTML in the live page. Reply:
        "Issue về element này cần team kỹ thuật kiểm tra trực tiếp trên store. Mình sẽ chuyển ticket sang team để các bạn xử lý. Trước đó, cho mình xin thêm vài thông tin nhé."

        STEP 2 — Collect:
        a) Editor link of the affected page. Ask: "Bạn gửi mình link editor của trang đang gặp lỗi nhé."
        b) Detailed description. Ask: "Bạn cho mình biết element nào đang lỗi (navigation, dropdown, slideshow, pagination, …) và hành vi lỗi cụ thể là gì nhé."
        c) Visual evidence (OPTIONAL but helpful): "Nếu được, bạn gửi mình ảnh chụp hoặc video ngắn cho thấy lỗi — bạn có thể paste link hoặc đính kèm file trực tiếp trong chat cũng được."
        d) Publish consent: "Khi team kỹ thuật fix xong, mình publish luôn trang lên hay bạn muốn mình chỉ save thôi?"

        STEP 3 — Have editor_link + detail + publish_status. BEFORE calling the tool, ask the customer to EXIT the editor and WAIT for explicit confirmation:
        ...
        a) Call escalate_element_notworking_issue with: issue_description (English, detailed), editor_link, publish_status, user_exited_editor=true. If user pasted screenshot URLs include them in screenshot_urls. If user attached files in chat set customer_attached_files=true. ALWAYS include customer_last_message_text.
      ...
      `,
      inputSchema: ESCALATE_ELEMENT_NOTWORKING_INPUT_SHAPE,
      outputSchema: ESCALATE_ELEMENT_NOTWORKING_OUTPUT_SHAPE,
    },
    async (input: EscalateElementNotworkingInput) => {
      const output: EscalateElementNotworkingOutput = await escalateElementNotworkingIssueHandler(input);
      return {
        content: [
          {
            type: "text",
...
export { registerEscalateElementNotworkingIssueTool };
