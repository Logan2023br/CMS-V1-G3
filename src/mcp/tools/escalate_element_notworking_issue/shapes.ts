...
const ESCALATE_ELEMENT_NOTWORKING_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's detailed English paraphrase. Identify WHICH element is broken (navigation / dropdown / slideshow / pagination / etc.) and WHAT the OBSERVED broken behavior is. Example: 'Navigation menu does not open when clicked on mobile.', 'Slideshow auto-advance does not work; arrows are unresponsive.', 'Pagination not showing on the blog grid.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Optional. URLs the user pasted showing the broken element (screenshot or short screen recording demonstrating the failure). Omit if the user attached files directly in chat."
    ),
...
type EscalateElementNotworkingInput = z.infer<typeof ESCALATE_ELEMENT_NOTWORKING_INPUT_SHAPE>;
...
const ESCALATE_ELEMENT_NOTWORKING_OUTPUT_SHAPE = z.object({
...
type EscalateElementNotworkingOutput = z.infer<typeof ESCALATE_ELEMENT_NOTWORKING_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateElementNotworkingInput,
  type EscalateElementNotworkingOutput,
};
