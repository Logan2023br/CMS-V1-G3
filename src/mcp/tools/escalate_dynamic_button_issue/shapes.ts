...
const ESCALATE_DYNAMIC_BUTTON_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST classify the symptom: (a) Dynamic Button / Buy Now not functioning (click does nothing / wrong destination / payment error), or (b) styling request (restyle appearance / colors / size). Example: 'Buy Now (Dynamic Button) does not trigger checkout on live; clicking does nothing.', 'Customer wants to restyle Dynamic Button: background #000, text white, bigger padding.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot or screen-recording URLs the user pasted showing the broken Dynamic Button behavior or the current vs desired styling. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true."
    ),
  ...
  user_consented_to_publish: z
    .boolean()
    .describe(
      "MUST be true. The user has explicitly agreed that the technical team may publish the page after fixing. TS team WILL publish (no save-only option — fix/restyle must be visible on live to verify). Ask first if you have not."
    ),
...
type EscalateDynamicButtonInput = z.infer<typeof ESCALATE_DYNAMIC_BUTTON_INPUT_SHAPE>;
...
const ESCALATE_DYNAMIC_BUTTON_OUTPUT_SHAPE = z.object({
...
type EscalateDynamicButtonOutput = z.infer<typeof ESCALATE_DYNAMIC_BUTTON_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateDynamicButtonInput,
  type EscalateDynamicButtonOutput,
};
