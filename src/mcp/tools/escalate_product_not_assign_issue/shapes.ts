...
const ESCALATE_PRODUCT_NOT_ASSIGN_INPUT_SHAPE = z.object({
  issue_description: z
    .string()
    .min(1)
    .describe(
      "Hugo's one-line paraphrase of the issue, ALWAYS IN ENGLISH. MUST note that the customer is the store OWNER (or has full product permissions) and the assign action still fails. Example: 'Customer is store owner, cannot assign product to PageFly product page; assign action fails silently.', 'Owner cannot assign collection to PageFly collection page; permissions not the cause.'"
    ),
  ...
  screenshot_urls: z
    ...
    .optional()
    .describe(
      "Screenshot or screen-recording URLs the user pasted showing the failed assign attempt. Optional in schema — but customer MUST provide visual evidence either as URL(s) OR via customer_attached_files=true."
    ),
...
type EscalateProductNotAssignInput = z.infer<typeof ESCALATE_PRODUCT_NOT_ASSIGN_INPUT_SHAPE>;
...
const ESCALATE_PRODUCT_NOT_ASSIGN_OUTPUT_SHAPE = z.object({
...
type EscalateProductNotAssignOutput = z.infer<typeof ESCALATE_PRODUCT_NOT_ASSIGN_OUTPUT_SHAPE>;
...
export {
  ...
  type EscalateProductNotAssignInput,
  type EscalateProductNotAssignOutput,
};
