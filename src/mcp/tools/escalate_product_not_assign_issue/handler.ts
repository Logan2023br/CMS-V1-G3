...
const MISSING_LABELS_EN: Record<MissingField, string> = {
  ...
  screenshot:
    "a screenshot or screen recording showing the failed assign action (paste link or attach the file directly in chat)",
  publish_status:
...
interface ProductNotAssignNoteFields {
...
function formatProductNotAssignNoteContent(
  fields: ProductNotAssignNoteFields,
  ticketUrl: string
): string {
...
async function escalateProductNotAssignIssueHandler(
  input: EscalateProductNotAssignInput,
  accessChecker: AccessChecker = requireStoreAccess
): Promise<EscalateProductNotAssignOutput> {
  ...
  if (!access.ready) {
    return {
      ...
      ...access.output,
    } as EscalateProductNotAssignOutput;
  }
  ...
  const noteResult: PostNoteResult = await tryPostNoteWithScoring({
    ...
    formatNote: formatProductNotAssignNoteContent,
  });

  if (noteResult.posted) {
    console.log(
      `[escalate_product_not_assign_issue] match: session=${noteResult.sessionUsed} source=${noteResult.sessionSource} score=${noteResult.match?.score ?? "n/a"} signals=[${noteResult.match?.signalsMatched.join(", ") ?? ""}] posted=true`
    );
  } else {
    console.error(
      `[escalate_product_not_assign_issue] match: posted=false error=${noteResult.error}`
    );
  }
...
export { escalateProductNotAssignIssueHandler, formatProductNotAssignNoteContent };
